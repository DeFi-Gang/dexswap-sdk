...
function computePoolAddress(_ref) {
  ...
  var _ref2 = tokenA.sortsBefore(tokenB) ? [tokenA, tokenB] : [tokenB, tokenA],
    token0 = _ref2[0],
    token1 = _ref2[1];
  return getCreate2Address(poolDeployer != null ? poolDeployer : POOL_DEPLOYER_ADDRESSES[tokenA.chainId], keccak256(['bytes'], [defaultAbiCoder.encode(['address', 'address'], [token0.address, token1.address])]), initCodeHashManualOverride != null ? initCodeHashManualOverride : POOL_INIT_CODE_HASH[tokenA.chainId]);
}
function computePoolAddressZkSync(_ref3) {
  var poolDeployer = _ref3.poolDeployer,
    tokenA = _ref3.tokenA,
    tokenB = _ref3.tokenB,
  ...
  var _ref4 = tokenA.sortsBefore(tokenB) ? [tokenA, tokenB] : [tokenB, tokenA],
    token0 = _ref4[0],
    token1 = _ref4[1]; // does safety checks
  return getCreate2AddressZk(poolDeployer != null ? poolDeployer : POOL_DEPLOYER_ADDRESSES[tokenA.chainId], keccak256(['bytes'], [defaultAbiCoder.encode(['address', 'address'], [token0.address, token1.address])]), initCodeHashManualOverride != null ? initCodeHashManualOverride : POOL_INIT_CODE_HASH[tokenA.chainId]);
}
function getCreate2AddressZk(from, salt, initCodeHash) {
  var prefix = keccak256$1(toUtf8Bytes('zksyncCreate2'));
  var addressBytes = keccak256$1(concat([prefix, zeroPad(from, 32), salt, initCodeHash, '0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470'])).slice(26);
...
var Pool = /*#__PURE__*/function () {
   ...
   */
  function Pool(tokenA, tokenB, fee, sqrtRatioX96, liquidity, tickCurrent, _tickSpacing, ticks) {
    if (ticks === void 0) {
      ...
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            return _context.abrupt("return", [CurrencyAmount.fromRawAmount(outputToken, JSBI.multiply(outputAmount, NEGATIVE_ONE)), new Pool(this.token0, this.token1, this.fee, sqrtRatioX96, liquidity, tickCurrent, this.tickSpacing, this.tickDataProvider)]);
          case 11:
          case "end":
            return _context.stop();
        }
      ...
      return _regeneratorRuntime().wrap(function _callee2$(_context2) {
        while (1) switch (_context2.prev = _context2.next) {
          ...
          case 4:
            ...
            return _context2.abrupt("return", [CurrencyAmount.fromRawAmount(inputToken, inputAmount), new Pool(this.token0, this.token1, this.fee, sqrtRatioX96, liquidity, tickCurrent, this.tickSpacing, this.tickDataProvider)]);
          case 11:
          case "end":
            return _context2.stop();
        }
...
var Position = /*#__PURE__*/function () {
  ...
  _proto.mintAmountsWithSlippage = function mintAmountsWithSlippage(slippageTolerance) {
    ...
    var poolLower = new Pool(this.pool.token0, this.pool.token1, this.pool.fee, sqrtRatioX96Lower, 0 /* liquidity doesn't matter */, TickMath.getTickAtSqrtRatio(sqrtRatioX96Lower), this.pool.tickSpacing);
    var poolUpper = new Pool(this.pool.token0, this.pool.token1, this.pool.fee, sqrtRatioX96Upper, 0 /* liquidity doesn't matter */, TickMath.getTickAtSqrtRatio(sqrtRatioX96Upper), this.pool.tickSpacing);
...
function tryParseAmount(value, currency) {
  ...
  try {
    var typedValueParsed = parseUnits(value, currency == null ? void 0 : currency.decimals).toString();
...
SelfPermit.INTERFACE = /*#__PURE__*/new Interface(selfPermitABI);
...
var NonfungiblePositionManager = /*#__PURE__*/function (_SelfPermit) {
  ...
  NonfungiblePositionManager.addCallParameters = function addCallParameters(position, options) {
    ...
    if (options.useNative) {
      ...
      !(position.pool.token0.equals(wrapped) || position.pool.token1.equals(wrapped)) ? process.env.NODE_ENV !== "production" ? invariant(false, 'NO_WNative') : invariant(false) : void 0;
  ...
  NonfungiblePositionManager.encodeCreate = function encodeCreate(pool) {
    return NonfungiblePositionManager.INTERFACE.encodeFunctionData('createAndInitializePoolIfNecessary', [pool.token0.address, pool.token1.address, toHex(pool.sqrtRatioX96)]);
  };
...
NonfungiblePositionManager.INTERFACE = /*#__PURE__*/new Interface(algebraPositionManagerABI);
...
SwapRouter.INTERFACE = /*#__PURE__*/new Interface(algebraSwapRouterABI);

export { ADDRESS_ZERO, Bound, ChainId, CurrencyAmount, DEFAULT_TICK_SPACING, ExtendedNative, Field, Fraction, FullMath, INITIAL_POOL_FEE, LiquidityMath, MaxUint128, MaxUint256, NEGATIVE_ONE, Native, NoTickDataProvider, NonfungiblePositionManager, ONE, POOL_DEPLOYER_ADDRESSES, POOL_INIT_CODE_HASH, Percent, Pool, Position, Price, Q192, Q96, RetryableError, Rounding, Route, SelfPermit, SqrtPriceMath, SwapRouter, Tick, TickList, TickListDataProvider, TickMath, Token, Trade, TradeType, WNATIVE, ZERO, algebraPositionManagerABI, algebraSwapRouterABI, computePoolAddress, computePoolAddressZkSync, currencyEquals, encodeRouteToPath, encodeSqrtRatioX96, getTickToPrice, isSorted, maxAmountSpend, maxLiquidityForAmounts, mostSignificantBit, nearestUsableTick, parseBalance, priceToClosestTick, retry, selfPermitABI, tickToPrice, toHex, tradeComparator, tryParseAmount, tryParsePrice, tryParseTick, unwrappedToken };
//# sourceMappingURL=integral-sdk.esm.js.map
